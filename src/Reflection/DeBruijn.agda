...
DeBruijnTraversable : ∀ {F : ∀ {a} → Set a → Set a} {{_ : Traversable F}}
                        {{_ : DeBruijn A}} → DeBruijn (F A)
...
instance
  DeBruijnℕ : DeBruijn ℕ
  strengthenFrom {{DeBruijnℕ}} = strVar
  weakenFrom     {{DeBruijnℕ}} = wkVar
